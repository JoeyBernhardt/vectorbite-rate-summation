...
king_fit_quad <- b_TPC(data = king_const, ## data
...
summary(king_fit_quad)

## We will repeat the model diagnostic steps
## Look at the trace plot of all the parameters
par(mfrow = c(2,2))
traceplot(king_fit_quad, burn = 1000)

## Look at the ACF of the chains
s1 <- as.data.frame(king_fit_quad$samples[1000:10000,])
par(mfrow = c(2,2))
for(i in 1:4) {
  acf(s1[,i], lag.max = 50, main = "",
      ylab = paste("ACF: ", names(s1)[i], sep =""))
...
## Compare the prior and the posterior distributions
par(mfrow = c(1,1))
ppo_plot(king_fit_quad, burn = 1000)

# Model selection ----
## pull the Watanabe–Akaike information criterion (WAIC) from the models
king_fit_briere$mcmc$getWAIC()
king_fit_quad$mcmc$getWAIC()
  

# Plot the fits ----
briere_pred <- posterior_predictive(king_fit_briere)
plot(briere_pred)
quad_pred <- posterior_predictive(king_fit_quad)

briere_fit <- as_tibble(summary(king_fit_briere, plot = F))
